...
- [ ] 1. 创建 Canvas State Manager 核心模块
  - [ ] 1.1 创建 `public/js/design/utils/canvas-state-manager.js` 文件
...
- [ ] 2. 实现状态保存功能
  - [ ] 2.1 实现 `saveViewState(viewId)` 方法
  ...
  - [ ] 2.2 实现防抖保存机制
    ...
    - _需求: 1.1_
  - [ ] 2.3 实现 `saveAllViewStates()` 方法
    - 遍历所有视图并保存状态
    - 保存 selectedColorsByView 数据
    - _需求: 5.1, 8.1_

- [ ] 3. 实现状态恢复功能
  - [ ] 3.1 实现 `restoreViewState(viewId)` 方法
...
- [ ] 4. 扩展 Pinia Stores
  - [ ] 4.1 扩展 Canvas Store（`public/js/design/stores/index.js`）
    - 添加 `isRestoringState: false` 状态
    - 添加 `setRestoringState(value)` 方法
    - 添加 `restoreViewData(viewId, { layers, layerGroups })` 方法
    - _需求: 2.2, 3.2_
  - [ ] 4.2 扩展 Print Method Store（`public/js/design/stores/printMethodStore.js`）
    - 添加 `restorePrintMethodMappings(viewId, { layerMap, groupMap })` 方法
    - _需求: 4.2_

- [ ] 5. 实现状态清除功能
  - [ ] 5.1 实现 `clearProductState(productId)` 方法
    - 从 LocalStorage 删除指定产品的所有数据
    - _需求: 7.1_
  - [ ] 5.2 实现 `clearViewState(viewId)` 方法
    - 从存储中删除指定视图的数据
    - _需求: 7.2, 7.3_
...
- [ ] 7. 集成到现有系统
  - [ ] 7.1 在页面初始化时初始化 CanvasStateManager
  ...
  - [ ] 7.6 监听颜色选择变更
    ...
    - 触发状态保存
    - _需求: 8.3_
  - [ ] 7.7 移除 header-controls.js 中的重复保存逻辑
